...
function RedisComplete(options) {
  ...
  this.openConn = options.conn || null

  this.conn = new Connection({port: this.port, host: this.host, auth: this.auth, conn: this.openConn});
...
RedisComplete.prototype.update = function (options, cb, mergecb) {
  var self = this;
  ...
  async.each(data, function (dataItem, cb) {
    self.conn.hget({key: self.app + ':' + ns + ':docs', field: dataItem[idField]}, function (err, item) {
      ...
      if (!err) {
        if (!item) {
          updated.push({ index: dataItem[idField], item: dataItem})
         // console.log(new Date().getTime() + ' item: ' + self.app + ':' + ns + ':docs' + ' field:' +  dataItem[idField] + ' was not found yet')
          self.add({data: [dataItem], ns: ns, idField: idField, complKey: complKey, sortKey: sortKey}, cb);
        } else {
          item = JSON.parse(item);
          var newItem = self.mergeDocs(item, dataItem, mergecb);
          updated.push({ index: dataItem[idField], item: newItem})
...
RedisComplete.prototype.search = function (options, cb) {
  ...
  if (terms.length > 1) {
    ...
    this.getIdMatches({search: prefixes, ns: options.datans || ns, limit: limit, offset: offset, count: options.count, reverse: options.reverse}, function (err, ids, count) {
      err && cb(err);
      if (!err) {
        if (!ids || ids.length === 0) {
          self.intersectCompletions({terms: terms, ns: ns, limit: limit, offset: offset}, function (err, result) {
            cb(err, result, count)
          });
        } else {
          self.getDocs({ids: ids, ns: options.datans || ns}, function (err, result) {
            cb(err, result, count)
          });
        }
      }
    ...
    this.getIdMatches({search: search, ns: ns, limit: limit, offset: offset, count: options.count, reverse: options.reverse}, function (err, ids, count) {
      (err && cb(err)) || self.getDocs({ids: ids, ns: options.datans || ns}, function (err, result) { cb(err, result, count) })
    });
  }
};
...
RedisComplete.prototype.getIdMatches = function (options, cb) {
  ...
  var offset = options.offset;
//console.log(options.reverse)
  this.conn.zrangebyscore({key: key, limit: limit, offset: offset, count: options.count, reverse: options.reverse}, cb);
};
  ...
  if (!options.hasOwnProperty('data') || (!(options.data instanceof Array))) {
    ...
    if (options.reset) {
      ...
      this.addDocuments({data: options.data, ns: options.ns, idField: options.idField}, function (err) {
        (err && cb(err)) || self.addCompletions(options, function(err){  cb(err) } );
      });
    }
  }
};
      ...
      if (err) {
        ...
        if (name && name.length > 0) {
          ...
          async.each(words, function (wordObj, cb) {
            ...
            var score = options.score || wordObj.score;
            var prefixes = [];
                  ...
                  self.conn.sadd({key: self.app + ':' + options.ns + ':' + id, value: { key: prefixEntry.prefix, value: sortName + ':' + id }}, function (err) {
                    ...
                    if (!err) {
                      if (prefixEntry.type === 'normal') {
                        ...
                        self.conn.zadd({key: prefixEntry.prefix, score: options.score || maxScore, value: sortName + ':' + id}, cb);
                      }
                    }
                  });
                }
...
RedisComplete.prototype.mergeDocs = function (oldDoc, newDoc, mergecb) {
  var finalDoc = {};
  ...
  if(mergecb)
    return mergecb(oldDoc, newDoc, finalDoc)
