  ...
  if (!options.hasOwnProperty('data') || (!(options.data instanceof Array))) {
    ...
    if (options.reset) {
      ...
      this.addDocuments({data: options.data, ns: options.ns, idField: options.idField}, function (err) {
        (err && cb(err)) || self.addCompletions(options, function(err){ console.log('done: ' + new Date().getTime() ); cb(err)} );
      });
    }
  }
};
...
RedisComplete.prototype.mergeDocs = function (oldDoc, newDoc, mergecb) {
  ...
  for (var oldItem in oldDoc) {
    ...
    /*else {
      if(Array.isArray(oldDoc[oldItem])) {
        console.log('----------------------------------------')
        console.log(oldDoc[oldItem])
        console.log(oldItem)
        finalDoc[oldItem] = this.mergeDocs(oldDoc[oldItem], finalDoc[oldItem]  )
      } 
      else if(oldDoc[oldItem] ) {
